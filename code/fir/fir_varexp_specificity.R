...
df.medians <- sapply(coef.lvls,function(x) 
  sapply(1:ncomps.plot, function(PC) median(df.plot$values[df.plot$Coeff.TS==x & df.plot$names ==paste0('PC',PC)])))
df.medians <- data.frame(df.medians,PC=paste0('PC',1:ncomps.plot),check.names = F)
...
p <- ggplot() + geom_col(data=df.medians,aes(x=PC,y=values,fill=names),position = position_dodge(1),alpha=0.5)+
  ...
  scale_x_discrete(limits=paste0('PC',1:ncomps.plot)) + 
